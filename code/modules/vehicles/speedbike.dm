...
/obj/vehicle/ridden/space/apccontrol
	name = "APC"
	desc = "Security's riot control vehicle. Don't even ask how they got this. If anyone else than security uses this vehicle, apply RPG repeatedly. If unable to, RUN, because otherwise it's going to drive you over and break all of your bones...and that's at best."
	icon = 'icons/obj/apc.dmi'
	icon_state = "apctankie"
...
/obj/vehicle/ridden/space/apccontrol/Initialize()
	. = ..()
	var/datum/component/riding/D = LoadComponent(/datum/component/riding)
	...
	D.set_vehicle_dir_offsets(WEST, -48, -48)
	for(var/i in GLOB.cardinals)
		D.set_vehicle_dir_layer(i, ABOVE_MOB_LAYER)

/obj/vehicle/ridden/space/apccontrol/Bump(atom/movable/A)
	. = ..()
	if(A.density && has_buckled_mobs())
		var/atom/throw_target = get_edge_target_turf(A, dir)
		if(crash_all)
		...
		if(ishuman(A))
			...
			H.Paralyze(100)
			H.adjustStaminaLoss(30)
			H.apply_damage(rand(75,95), BRUTE)
			if(!crash_all)
				H.throw_at(throw_target, 3, 3)
				visible_message("<span class='danger'>[src] crashes into [H]!</span>")
				playsound(src, 'sound/effects/bang.ogg', 50, 1)

/obj/vehicle/ridden/space/apccontrol/Moved()
	. = ..()
	if(has_buckled_mobs())
		for(var/atom/A in range(2, src))
			if(!(A in buckled_mobs))
